<div align="center">

[![header](https://capsule-render.vercel.app/api?type=waving&height=280&color=0:0A0F1E,40:0E1E45,80:152454,100:0A0F1E&text=Shahram%20Munsaf&fontSize=58&fontColor=E8D5B0&fontAlignY=42&desc=Full-Stack%20Developer%20%E2%80%82%C2%B7%E2%80%82AI%20Engineer%20%E2%80%82%C2%B7%E2%80%82Teaching%20Assistant&descSize=17&descColor=8AA4CC&animation=fadeIn&stroke=2A3F6F&strokeWidth=1)](https://github.com/Smmedy06)

</div>

<div align="center">

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=16&duration=3000&pause=800&color=8AA4CC&center=true&vCenter=true&width=600&height=32&lines=BSCS+%40+PUCIT+Lahore+%E2%80%94+Semester+6+%E2%80%94+CGPA+3.81+%2F+4.0;Building+intelligent+systems+that+actually+ship.;Open+to+internships+in+SWE+%7C+AI+%7C+Full-Stack" alt="status line" />

</div>

<br>

<div align="center">

[![GitHub](https://img.shields.io/badge/-Smmedy06-E8D5B0?style=flat-square&logo=github&logoColor=0A0F1E)](https://github.com/Smmedy06)&ensp;[![LinkedIn](https://img.shields.io/badge/-shahram--munsaf-4A7FBB?style=flat-square&logo=linkedin&logoColor=white)](https://linkedin.com/in/shahram-munsaf)&ensp;[![Email](https://img.shields.io/badge/-smmedy06@gmail.com-6DAF8A?style=flat-square&logo=gmail&logoColor=white)](mailto:smmedy06@gmail.com)

![Profile Views](https://komarev.com/ghpvc/?username=Smmedy06&color=8AA4CC&style=flat-square&label=Profile+Views)&ensp;![Followers](https://img.shields.io/github/followers/Smmedy06?style=flat-square&color=4A7FBB&logo=github&label=Followers)

</div>

---

## Who I Am

CS undergraduate at PUCIT building production-grade systems across the full stack — from MERN and Spring Boot backends to LLM-powered agentic pipelines. I teach DSA and COAL as a Teaching Assistant, which keeps me sharp on fundamentals while I push forward on AI engineering. Currently focused on making intelligent systems that actually ship.

---

## Stack

**Languages**

[![Skills](https://skillicons.dev/icons?i=c,cpp,java,python,js,html,css&theme=dark)](https://skillicons.dev)

**Frontend**

[![Skills](https://skillicons.dev/icons?i=react,figma,vite&theme=dark)](https://skillicons.dev)

**Backend & APIs**

[![Skills](https://skillicons.dev/icons?i=nodejs,express,fastapi,flask,spring&theme=dark)](https://skillicons.dev)

**Databases**

[![Skills](https://skillicons.dev/icons?i=mysql,postgres,mongodb,firebase,supabase&theme=dark)](https://skillicons.dev)

**AI / ML**

[![Skills](https://skillicons.dev/icons?i=python,sklearn&theme=dark)](https://skillicons.dev)
&nbsp;
![Pandas](https://img.shields.io/badge/Pandas-0D1B3E?style=flat-square&logoColor=E8D5B0)
![NumPy](https://img.shields.io/badge/NumPy-0D1B3E?style=flat-square&logoColor=4A7FBB)
![LLM Integration](https://img.shields.io/badge/LLM%20Integration-Agentic%20AI-6DAF8A?style=flat-square&logoColor=white)

**Tools**

[![Skills](https://skillicons.dev/icons?i=git,github,figma,vscode,jira&theme=dark)](https://skillicons.dev)

---

## Expertise

| Domain | Level | Details |
|---|---|---|
| Full-Stack Development | Advanced | MERN · Spring Boot · REST · FastAPI · Flask |
| Agentic AI & LLM | Intermediate–Advanced | Agentic pipelines · LLM APIs · narrative generation |
| Machine Learning | Intermediate | Scikit-learn · Pandas · NumPy · Matplotlib |
| Database Engineering | Advanced | MySQL · PostgreSQL · MongoDB · Firebase · Supabase |
| UI/UX Design | Intermediate | Figma · wireframing · Hotjar · MS Clarity |
| Teaching & Mentoring | Advanced | DSA · COAL · lab design · grading · mentoring |

---

## Projects

<details open>
<summary><b>Retail ERP System</b></summary>
<br>

Enterprise Resource Planning system covering customer management, inventory, finance tracking, and employee records — designed for a retail business with a modular, scalable architecture.

| | |
|---|---|
| **Stack** | Java · Spring Boot · SQL |
| **Architecture** | Modular — built for growth |
| **Modules** | Customers · Inventory · Finance · HR |
| **Role** | Solo — system design, backend, schema |

![Java](https://img.shields.io/badge/Java-0D1B3E?style=flat-square&logo=java&logoColor=E8D5B0)
![Spring Boot](https://img.shields.io/badge/Spring%20Boot-0A0F1E?style=flat-square&logo=springboot&logoColor=6DAF8A)
![SQL](https://img.shields.io/badge/SQL-0D1B3E?style=flat-square&logo=mysql&logoColor=4A7FBB)

</details>

<details>
<summary><b>Story to Scenes Generator</b></summary>
<br>

Agentic AI application that transforms a text prompt into a fully illustrated storybook — LLM-generated narrative fed into an image synthesis pipeline to produce a complete visual canvas.

| | |
|---|---|
| **Stack** | Python · FastAPI · LLM APIs |
| **Pipeline** | Prompt → LLM narrative → AI image synthesis |
| **Output** | Illustrated storybook canvas |
| **Role** | Full system — prompt engineering, backend, AI integration |

![Python](https://img.shields.io/badge/Python-0D1B3E?style=flat-square&logo=python&logoColor=E8D5B0)
![FastAPI](https://img.shields.io/badge/FastAPI-0A0F1E?style=flat-square&logo=fastapi&logoColor=6DAF8A)
![LLM](https://img.shields.io/badge/LLM%20APIs-0D1B3E?style=flat-square&logoColor=4A7FBB)

</details>

<details>
<summary><b>Crime Reporting System</b></summary>
<br>

Python desktop application with computer vision for crime reporting and incident management — secure auth, role-based access, and structured data handling for law enforcement workflows.

| | |
|---|---|
| **Stack** | Python · OpenCV · SQL |
| **Features** | Secure auth · Role-based access · Incident management |
| **CV Module** | Evidence and image handling |
| **Repo** | [criminal-database-management-system](https://github.com/Smmedy06/criminal-database-management-system) |

![Python](https://img.shields.io/badge/Python-0D1B3E?style=flat-square&logo=python&logoColor=E8D5B0)
![OpenCV](https://img.shields.io/badge/OpenCV-0A0F1E?style=flat-square&logo=opencv&logoColor=6DAF8A)
![SQL](https://img.shields.io/badge/SQL-0D1B3E?style=flat-square&logo=mysql&logoColor=4A7FBB)

</details>

<details>
<summary><b>Student Portal</b></summary>
<br>

Full-stack web application for student management, course registration, and grade tracking built on the MERN stack with Supabase for auth and backend services.

| | |
|---|---|
| **Stack** | MongoDB · Express.js · React.js · Node.js · Supabase |
| **Features** | Student management · Course registration · Grade tracking |
| **Auth** | Supabase authentication |
| **Role** | Full-stack — frontend, backend, database schema |

![React](https://img.shields.io/badge/React-0D1B3E?style=flat-square&logo=react&logoColor=E8D5B0)
![Node.js](https://img.shields.io/badge/Node.js-0A0F1E?style=flat-square&logo=nodedotjs&logoColor=6DAF8A)
![Supabase](https://img.shields.io/badge/Supabase-0D1B3E?style=flat-square&logo=supabase&logoColor=4A7FBB)

</details>

---

## Experience

**Teaching Assistant — DSA & COAL**
Punjab University College of Information Technology · 2025 – 2026

Delivering lab sessions and designing problem sets for Data Structures & Algorithms and Computer Organization & Assembly Language. Responsible for grading, solution keys, and direct mentoring of junior students.

---

**Frontend Developer Intern & UI/UX Designer**
Technoid Horizon, Lahore · Dec 2024 – Mar 2025

Built responsive React.js interfaces across client projects, conducted usability analysis with Hotjar and Microsoft Clarity, and created wireframes and prototypes in Figma.

---

## Education

**Bachelor of Science in Computer Science**
Punjab University College of Information Technology (PUCIT), Lahore
2023 – 2027 · Semester 6 · CGPA 3.81 / 4.0

Relevant coursework: Data Structures & Algorithms · OOP · DBMS · Operating Systems · Software Engineering · SQA · Machine Learning · Artificial Intelligence

---

## GitHub Analytics

<div align="center">

<img height="175em" src="https://github-readme-stats.vercel.app/api?username=Smmedy06&show_icons=true&theme=transparent&title_color=E8D5B0&icon_color=6DAF8A&text_color=8AA4CC&bg_color=0D1117&border_color=1E2D4E&include_all_commits=true&count_private=true" />

<img height="175em" src="https://github-readme-streak-stats.herokuapp.com/?user=Smmedy06&theme=transparent&ring=E8D5B0&fire=6DAF8A&currStreakLabel=E8D5B0&sideLabels=8AA4CC&dates=4A6080&background=0D1117&border=1E2D4E&currStreakNum=ffffff&sideNums=4A7FBB" />

</div>

<div align="center">

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Smmedy06&layout=compact&theme=transparent&title_color=E8D5B0&text_color=8AA4CC&bg_color=0D1117&border_color=1E2D4E&langs_count=8" />

</div>

---

## Activity

<div align="center">

[![Activity Graph](https://github-readme-activity-graph.vercel.app/graph?username=Smmedy06&bg_color=0D1117&color=E8D5B0&line=4A7FBB&point=6DAF8A&area=true&area_color=1E2D4E&hide_border=false&border_color=1E2D4E&custom_title=Contribution%20Activity)](https://github.com/ashutosh00710/github-readme-activity-graph)

</div>

---

## Summary

<div align="center">

[![Profile Details](https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=Smmedy06&theme=github_dark)](https://github.com/vn7n24fzkq/github-profile-summary-cards)

</div>

<div align="center">

[![Repos Per Language](https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=Smmedy06&theme=github_dark)](https://github.com/vn7n24fzkq/github-profile-summary-cards)
[![Most Commit Language](https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=Smmedy06&theme=github_dark)](https://github.com/vn7n24fzkq/github-profile-summary-cards)

</div>

---

## Contribution Grid

<div align="center">

![Snake animation](https://raw.githubusercontent.com/Smmedy06/Smmedy06/output/github-contribution-grid-snake-dark.svg)

</div>

---

## Currently

```yaml
building:
  - "Retail ERP System — Java / Spring Boot / SQL"
  - "Story to Scenes Generator — Agentic AI / FastAPI / LLM APIs"

learning:
  - "Advanced LLM integration and agentic pipeline architecture"
  - "Spring Boot microservices and production backend patterns"
  - "ML model deployment with Scikit-learn and FastAPI"

open_to:
  - "Software Engineering Internships"
  - "Full-Stack Development roles"
  - "AI / ML Engineering positions"
  - "Research collaborations in AI and NLP"
```

---

<div align="center">

[![footer](https://capsule-render.vercel.app/api?type=waving&height=100&color=0:0A0F1E,40:0E1E45,100:0A0F1E&section=footer)](https://github.com/Smmedy06)

</div>
